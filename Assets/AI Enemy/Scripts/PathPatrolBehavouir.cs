...
public class PathPatrolBehavouir : AIBehaviour
{
    public PatrollPath patrolPath;
    [Range(0.1f, 1)]
    public float arriveDis = 1;
    ...
    {
        if (patrolPath == null)
            patrolPath = GetComponentInChildren<PatrollPath>();
    }
    ...
    public override void PerformAction(EnemyController tank, AIDetector detector)
    {
        if (!isWaiting)
        {
            ...
            if (Vector2.Distance(tank.transform.position, currentPatrolTarget)<arriveDis)
            {
                isWaiting = true;
                StartCoroutine(WaitCourine());
                return;
            }
